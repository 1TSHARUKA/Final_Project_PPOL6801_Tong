...
def metadata_all_cases(corpus):
    seen_advocates = {}
    name2gender = create_load_lookupname2gender()
    caseid2gender = parse_gender(corpus, name2gender, verbose=False, start_year=1980)
    caseid2stuff = utils.load_case_file()
    for conv in corpus.iter_conversations():
        utt_ids = conv.get_utterance_ids()
        if not utt_ids:
            continue
        case_id = conv.get_utterance(utt_ids[0]).meta.get("case_id", "")
        year = int(case_id.split("_")[0])
        if year < 2010 or year > 2014:
            continue
        utt_list = print_prev_utt_for_chunk(corpus, caseid2stuff, case=case_id)
        seen_advocates = analyzechunks(corpus, caseid2stuff, name2gender, caseid2gender, utt_list, seen_advocates)
...
if __name__ == '__main__':
    ...
    # Load the entire corpus once
    corpus = Corpus(filename="/Users/tyf981126/interruptions-supreme-court/supreme-corpus")

    # Run the metadata extraction over the full corpus
    metadata_all_cases(corpus)
